...
The main projects page will show featured projects.
  ...
  ```sh
   ...
   ```
Upon loading the main projects page, it will *always* drop the projects table to ensure it is up to date, and then will call the function `project_check()` which will check if there  convert the `project_name.md` file to a SQL table named projects, located in `portfolio.db`
