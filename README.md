...
After `project_check()` has completed, the categories of projects (c, python, web, or other) will be split into their own python dictionaries which will be included when finally `render_template("projects.html" c_projects=c_projects ...)` is returned.
When clicking on "View More Projects in ______",  
